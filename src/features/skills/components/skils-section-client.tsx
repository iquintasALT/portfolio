...
import OrbitingSkills from "@/features/skills/components/orbiting-skills";
import SkillsGrid from "@/features/skills/components/skill-grid";
  ...
  return (
    <section id={id} className="flex flex-col items-center overflow-hidden justify-start pt-20 min-h-screen px-2">
      <div className="w-full max-w-6xl mx-auto flex flex-col items-center gap-8">
        <div
          className="flex flex-row items-center gap-4 w-full justify-center mb-2 mt-0 sm:mt-2"
          ...
          <div className="flex gap-2 items-center ml-2">
            {LAYOUTS.map((opt) => (
              <Button
                ...
                variant={layout === opt.key ? "default" : "outline"}
                size="default"
              >
                    ...
                    >
                      <div className="flex flex-col items-center w-full">
                        <div className="w-full flex items-center gap-2 mb-3">
                          <span className="text-lg sm:text-xl font-bold text-indigo-300 tracking-wide drop-shadow-md pl-1">
                            {skills[activeSlide].category}
                          </span>
                          <span className="flex-1 border-b border-zinc-800/60 ml-2" />
                        </div>
                        {skills[activeSlide] && (
                          <SkillsGrid
                            skills={[skills[activeSlide]]}
                            className="[&_.SkillDisplay]:w-16 [&_.SkillDisplay]:h-16 [&_.SkillDisplay]:p-2 [&_.SkillDisplay_img]:w-7 [&_.SkillDisplay_img]:h-7"
                          />
                        )}
                      </div>
                    </motion.div>
